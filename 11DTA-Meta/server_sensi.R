...
est.srocsa <- function(p,c1.square=0.5,fix.c=TRUE){
  if(length(esting_omg[[paste0(fix.c,c1.square,beta0(),paste0(beta(),collapse=""),paste0(alpha(),collapse=""),input$Sauc1,input$allsingle,studyId())]])==0)
  {
    srocsa.rf <- srocsa(data.cc(),
    p,
    fix.c=fix.c,
    c1.square=c1.square,
    beta0=beta0(),
    ...
    sauc.type =  input$Sauc1,correct.type = input$allsingle)
#calculate
    esting_omg[[paste0(fix.c,c1.square,beta0(),paste0(beta(),collapse=""),paste0(alpha(),collapse=""),input$Sauc1,input$allsingle,studyId())]]<-srocsa.rf
    #memo
    rf<-c(p=p,srocsa.rf@dta[[which(srocsa.rf@p == p)]])
    esting_omg[[paste0(p,fix.c,c1.square,beta0(),paste0(beta(),collapse=""),paste0(alpha(),collapse=""),input$Sauc1,input$allsingle,studyId())]]<-rf
    rf#add
  }
  else
  {
    srocsa.rf <- srocsa(esting_omg[[paste0(fix.c,c1.square,beta0(),paste0(beta(),collapse=""),paste0(alpha(),collapse=""),input$Sauc1,input$allsingle,studyId())]],
    p,
    fix.c=fix.c,
    c1.square=c1.square,
    beta0=beta0(),
...
est.rfc <-function(p.seq,c1.square,fix.c=TRUE){
  ...
  if(length(esting_omg[[paste0(p,fix.c,c1.square,beta0(),paste0(beta(),collapse=""),paste0(alpha(),collapse=""),input$Sauc1,input$allsingle,studyId())]])==0) est.srocsa(p,c1.square,fix.c)
})}##caluculating force add
